...
- `raw` 保存 Git 版本化证据；新增材料不覆盖现有文件，受审阅的更新由检查点保留前后状态；
- `source` 随对来源理解的加深而更新；
...
| 位置 | 职责 | 是否进入页面索引 |
|---|---|---|
| `raw/` | Git 版本化证据文件，如 PDF、XLSX、图片和网页快照 | 否 |
...
`.gitattributes` 保证 `raw/**` 按二进制字节跟踪。所有 raw，包括 PDF 和电子表格，直接进入 Git 提交，因此任一检查点都能恢复当时的路径、字节及对应页面。`add` 不覆盖现有文件；批注、附件完善或资料替换可以通过受审阅的 `raw-update` 修改、改名或删除已提交 raw。

Python 比较 base 与当前或候选状态，将 raw 改名视为旧路径删除和新路径新增，并读取两侧引用图的并集。图关系包括 source 的 raw 所有权、页面正文中的直接 raw 链接、`sources` 属性和有效正文 wikilink；输出 raw 状态与前后 blob OID，以及全部潜在受影响 source/note 的最短距离、前驱、后继、循环组和稳定分层。循环引用折叠为同层审核组。Python 只计算结构，不读取或概括页面语义，也不判断实际影响。

LLM 先比较 raw 前后内容，再只读取第 1 层页面。每层对页面或循环组判断 `none`、`link-only`、`update`、`rewrite` 或 `delete`：`none` 和不改变页面身份的普通 `link-only` 停止该分支；语义更新、重写、删除或身份变化才激活下游。页面有多个前驱时，任一前驱继续传播就进入审核，全部前驱停止时自动跳过。当前层没有继续传播的判断时立即结束；不得预读完整候选或打开被剪枝页面来确认无影响。

候选检查点显式纳入变更 raw、原 owner source 和实际修改页面；owner source 审核后可以保持字节不变。保存预览重新显示完整影响图与候选差异，用户批准后原子形成检查点。未修复的 raw 所有权、来源关系或 wikilink 继续阻止保存；二进制冲突必须先由用户确定最终文件。
...
| 需审阅 | 修改、改名或删除已提交 raw，人工编辑检查点、全库标签规范化、页面改名或删除、合并、拆分、大幅改写用户内容、改变来源关系、解决冲突 | 展示影响图与差异，用户确认后写入或提交 |
| 禁止 | 未经逐层审核修改已提交 raw、丢弃未确认修改、提交无关文件、由 Python 编造语义元数据或影响判断 | 停止并说明原因 |
...
| `begin` | 当前 vault | 返回基线、dirty 状态和需要先处理的人类或 LLM 变更；raw 变化时返回完整分层影响图 |
...
- 修改或删除已提交 raw 时，`save` 只接受高风险操作类型 `raw-update`，并要求明确纳入变更 raw 与每个原 owner source；预览和保存均返回候选影响图；
...
- 固定目录、元数据、raw 版本化与逐层影响审核和 Git 检查点合同；
...
- 实现 base 与当前或候选引用图并集、最短距离、循环组和稳定分层；
- 覆盖成功、冲突和失败保持状态；
- 保证相同输入生成相同字节结果。
...
- 完全重复的材料复用已有 raw，新增材料不会覆盖现有文件，二进制字节不受 Git 文本过滤影响。
- 已提交 raw 的修改、改名或删除只通过经批准的 `raw-update` 保存；当前提交包含新 raw 状态和实际受影响页面，父提交仍能恢复旧 raw 与旧页面状态。
- `begin` 与候选 `save` 对同一变化返回确定性的完整影响图，raw 改名表示为删除与新增，删除或移除链接后仍保留 base owner 与旧依赖候选。
- LLM 只打开当前有效的最近层。近层判断 `none` 或普通 `link-only` 时，其仅由该分支到达的远层自动跳过；任一前驱继续传播时才审核多前驱页面；循环组不会重复或无限审核。
- 非 `raw-update` 操作不能提交已存在 raw 的修改或删除；缺少原 owner source 的明确 include、未修复引用或未通过审计都会阻止保存。
...
2. 摄入中文 PDF 并创建来源页；存在耐久知识增量时创建或更新笔记；重复摄入复用 raw，每个检查点保留精确字节。
...
7. 用户为已提交 PDF 增加批注；系统一次计算完整影响图，LLM 只审核第 1 层 owner source。若判断 `none`，下游笔记不被读取，raw 以受批准的 `raw-update` 保存，父提交仍可恢复旧 PDF。
